...
public class XMLSchemaLoader implements SchemaLoader {
    ...
    for (int i = 0; i < nodeList.getLength(); i++) {
      ...
      for (int j = 0; j < tableNames.length; j++) {
        ...
        TableConfig table = new TableConfig(tableName, primaryKey, autoIncrement, needAddLimit,
            tableType,
            dataNode, getDbType(dataNode), (tableRule != null) ? tableRule.getRule() : null,
            ruleRequired,
  ...
  /**
   * distribute datanodes in multi hosts,means ,dn1 (host1),dn100 (host2),dn300(host3),dn2(host1),dn101(host2),dn301(host3)...etc
   *
   * @param dataNodes
   */
  ...
  private void loadDataNodes(Element root) {
    ...
    for (int i = 0, n = list.getLength(); i < n; i++) {
      ...
      if (empty(dnNamePre) || empty(databaseStr) || empty(host)) {
        throw new ConfigException(
            "dataNode " + dnNamePre + " define error ,attribute can't be empty");
      }
      String[] dnNames = com.zhongan.dmds.commons.util.SplitUtil.split(dnNamePre, ',', '$', '-');
      String[] databases = com.zhongan.dmds.commons.util.SplitUtil
          .split(databaseStr, ',', '$', '-');
      String[] hostStrings = com.zhongan.dmds.commons.util.SplitUtil.split(host, ',', '$', '-');
  ...
  private DBHostConfig createDBHostConf(String dataHost, Element node, String dbType,
      String dbDriver, int maxCon,
    ...
    int weight = "".equals(weightStr) ? DmdsConstants.WEIGHT : Integer.valueOf(weightStr);

    String ip = null;
    int port = 0;
    if (empty(nodeHost) || empty(nodeUrl) || empty(user)) {
      throw new ConfigException(
          "dataHost " + dataHost + " define error,some attributes of this element is empty: "
              + nodeHost);
    }
  ...
  private void loadDataHosts(Element root) {
    ...
    for (int i = 0, n = list.getLength(); i < n; ++i) {
      ...
      int writeType =
          "".equals(writeTypStr) ? DmdsConstants.WRITE_ONLYONE_NODE : Integer.valueOf(writeTypStr);
      ...
      long logTime = "".equals(logTimeStr) ? DmdsConstants.LONG_TIME : Long.valueOf(logTimeStr);
      ...
      for (int w = 0; w < writeDbConfs.length; w++) {
        ...
        writeDbConfs[w] = createDBHostConf(name, writeNode, dbType, dbDriver, maxCon, minCon,
            filters, logTime);
        ...
        if (readNodes.getLength() != 0) {
          ...
          for (int r = 0; r < readDbConfs.length; r++) {
            ...
            readDbConfs[r] = createDBHostConf(name, readNode, dbType, dbDriver, maxCon, minCon,
                filters, logTime);
          }
