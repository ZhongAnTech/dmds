...
public class StringUtil {
  ...
  /**
   * 字符串hash算法：s[0]*31^(n-1) + s[1]*31^(n-2) + ... + s[n-1] <br> 其中s[]为字符串的字符数组，换算成程序的表达式为：<br> h =
   * 31*h + s.charAt(i); => h = (h << 5) - h + s.charAt(i); <br>
   *
      ...
      if (ch == '/' && pos + 4 < sql.length() && sql.charAt(pos + 1) == '*') {
        ...
        if (intoFound) {
          ...
          if (pos + 5 < sql.length() && (sql.charAt(pos + 1) == 'n' || sql.charAt(pos + 1) == 'N')
              && (sql.charAt(pos + 2) == 't' || sql.charAt(pos + 2) == 'T')
              && (sql.charAt(pos + 3) == 'o' || sql.charAt(pos + 3) == 'O')
              && (sql.charAt(pos + 4) <= ' ')) {
            pos = pos + 5;
            intoFound = true;
          } else {
          ...
          if (pos + 7 < sql.length() && (sql.charAt(pos + 1) == 'n' || sql.charAt(pos + 1) == 'N')
              && (sql.charAt(pos + 2) == 's' || sql.charAt(pos + 2) == 'S')
              && (sql.charAt(pos + 3) == 'e' || sql.charAt(pos + 3) == 'E')
              && (sql.charAt(pos + 4) == 'r' || sql.charAt(pos + 4) == 'R')
              && (sql.charAt(pos + 5) == 't' || sql.charAt(pos + 5) == 'T')
              && (sql.charAt(pos + 6) <= ' ')) {
            pos = pos + 7;
            insertFound = true;
          } else {
  ...
  public static void main(String[] args) {
    ...
    System.out.println(getTableName(
        "/* ApplicationName=DBeaver 3.3.1 - Main connection */ insert into employee(id,name,sharding_id) values(4,’myhome’,10011)"));
