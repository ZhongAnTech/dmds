...
import com.zhongan.dmds.core.LoadDataInfileHandler;
...
import com.zhongan.dmds.sqlParser.druid.DmdsStatementParser;
...
/**
 * mysql命令行客户端也需要启用local file权限，加参数--local-infile=1 jdbc则正常，不用设置 load data sql中的CHARACTER SET 'gbk'
 * 其中的字符集必须引号括起来，否则druid解析出错
 */
public final class ServerLoadDataInfileHandler implements LoadDataInfileHandler {
  ...
  private void parseLoadDataPram() {
    ...
    String charset =
        statement.getCharset() != null ? statement.getCharset() : serverConnection.getCharset();
  ...
  public void start(String sql) {
    ...
    SQLStatementParser parser = new DmdsStatementParser(sql);
    ...
    schema = DmdsContext.getInstance().getConfig().getSchemas().get(serverConnection.getSchema());
    tableId2DataNodeCache = (LayerCachePool) DmdsContext.getInstance().getCacheService()
    ...
    tempPath =
        SystemConfig.getHomePath() + File.separator + "temp" + File.separator + serverConnection
            .getId()
            + File.separator;
  ...
  private synchronized void saveByteOrToFile(byte[] data, boolean isForce) {
    ...
    // 超过200M 存文件
    if ((isForce && isHasStoreToFile) || tempByteBuffrSize > 200 * 1024 * 1024) {
  ...
  private RouteResultset tryDirectRoute(String sql, String[] lineList) {
    ...
    if (tableConfig == null && schema.getDataNode() != null) {
      // 走默认节点
      RouteResultsetNode rrNode = new RouteResultsetNode(schema.getDataNode(), ServerParse.INSERT,
          sql);
      ...
      if (partitionColumnIndex == -1 || partitionColumnIndex >= lineList.length) {
        ...
        try {
          ...
          for (RouteCalculateUnit unit : ctx.getRouteCalculateUnits()) {
            RouteResultset rrsTmp = DmdsRouterUtil.tryRouteForTables(schema, ctx, unit, rrs, false,
  ...
  private RouteResultset buildResultSet(Map<String, LoadData> routeMap) {
    ...
    for (String dn : routeMap.keySet()) {
      RouteResultsetNode rrNode = new RouteResultsetNode(dn, ServerParse.LOAD_DATA_INFILE_SQL,
          srcStatement);
      ...
      // if (isHasStoreToFile)
      if (loadData1.getFileName()
          != null)// 此处判断是否有保存分库load的临时文件dn1.txt/dn2.txt，不是判断是否有clientTemp.txt
      {
