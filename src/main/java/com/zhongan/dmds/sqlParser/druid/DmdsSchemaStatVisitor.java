...
/**
 * Base on MycatSchemaStatVisitor and RouterUtil
 * Druid解析器中用来从ast语法中提取表名、条件、字段等的vistor
 */
public class DmdsSchemaStatVisitor extends MySqlSchemaStatVisitor {
    ...
    if (tableStats.size() == 1) {// 只有一个表，直接返回这一个表名
      ...
      if (parent instanceof MySqlSelectQueryBlock) {
        ...
        if (select.getFrom() instanceof SQLJoinTableSource) {// 多表连接
          ...
          return joinTableSource.getLeft()
              .toString();// 将left作为主表，此处有不严谨处，但也是实在没有办法，如果要准确，字段前带表名或者表的别名即可
  ...
  public boolean visit(SQLBinaryOpExpr x) {
    ...
    switch (x.getOperator()) {
      ...
      case BooleanOr:
        // 永真条件，where条件抛弃
        if (!DmdsSchemaStatVisitor.isConditionAlwaysTrue(x)) {
          hasOrCondition = true;
  ...
  private void mergeOneWhereUnit(WhereUnit whereUnit) {
    if (whereUnit.getSubWhereUnit().size() > 0) {
      ...
      if (whereUnit.getSubWhereUnit().size() > 1) {
        List<List<Condition>> mergedConditionList = getMergedConditionList(
            whereUnit.getSubWhereUnit());
  ...
  private void addExprIfNotFalse(WhereUnit whereUnit, SQLExpr expr) {
    // 非永假条件加入路由计算
    if (!DmdsSchemaStatVisitor.isConditionAlwaysFalse(expr)) {
