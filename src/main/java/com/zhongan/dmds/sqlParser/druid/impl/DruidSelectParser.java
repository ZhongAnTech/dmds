...
import com.zhongan.dmds.commons.util.ObjectUtil;
...
import com.zhongan.dmds.route.HavingCols;
...
public class DruidSelectParser extends DefaultDruidParser {
  ...
  protected static final Logger LOGGER = LoggerFactory.getLogger(DruidSelectParser.class);
  ...
  public void statementParse(SchemaConfig schema, RouteResultset rrs, SQLStatement stmt) {
    ...
    if (sqlSelectQuery instanceof MySqlSelectQueryBlock) {
      MySqlSelectQueryBlock mysqlSelectQuery = (MySqlSelectQueryBlock) selectStmt.getSelect()
          .getQuery();
  ...
  protected Map<String, String> parseAggGroupCommon(SchemaConfig schema, SQLStatement stmt,
      RouteResultset rrs,
  ...
  private boolean isRoutMultiNode(SchemaConfig schema, RouteResultset rrs) {
    ...
    LayerCachePool tableId2DataNodeCache = (LayerCachePool) DmdsContext.getInstance()
        .getCacheService()
  ...
  private String getFieldName(SQLSelectItem item) {
    if ((item.getExpr() instanceof SQLPropertyExpr) || (item
        .getExpr() instanceof SQLMethodInvokeExpr)
        || (item.getExpr() instanceof SQLIdentifierExpr) || item
        .getExpr() instanceof SQLBinaryOpExpr) {
  ...
  public void changeSql(SchemaConfig schema, RouteResultset rrs, SQLStatement stmt,
    ...
    if (sqlSelectQuery instanceof MySqlSelectQueryBlock) {
      ...
      if (groupByClause != null && groupByClause.getHaving() != null && isRoutMultiNode(schema,
          rrs)) {
      ...
      if (limit != null && !isNeedAddLimit) {
        ...
        if (isNeedChangeLimit(rrs)) {
          ...
          rrs.changeNodeSqlAfterAddLimit(schema, getCurentDbType(), getCtx().getSql(),
              rrs.getLimitStart(),
  ...
  private void tryRoute(SchemaConfig schema, RouteResultset rrs, LayerCachePool cachePool)
    ...
    if ((ctx.getTables() == null || ctx.getTables().size() == 0)
      ...
      rrs = DmdsRouterUtil.routeToSingleNode(rrs, schema.getRandomDataNode(), ctx.getSql());
      rrs.setFinishedRoute(true);
      return;
    }
    SortedSet<RouteResultsetNode> nodeSet = new TreeSet<RouteResultsetNode>();
    boolean isAllGlobalTable = DmdsRouterUtil.isAllGlobalTable(ctx, schema);

    if (LOGGER.isDebugEnabled()) {
      LOGGER.debug("RouteCalculateUnit:" + JSON.toJSONString(ctx.getRouteCalculateUnits()));
    }

    for (RouteCalculateUnit unit : ctx.getRouteCalculateUnits()) {
      RouteResultset rrsTmp = DmdsRouterUtil
          .tryRouteForTables(schema, ctx, unit, rrs, true, cachePool);
    ...
    if (tc == null || (ctx.getTables().size() == 1 && tc.isGlobalTable())) {
      ...
      if (ctx.getTables().size() == 1) {
        ...
        if (ctx.getRouteCalculateUnit().getTablesAndConditions().get(tableName) != null
            && ctx.getRouteCalculateUnit().getTablesAndConditions().get(tableName).get(primaryKey)
            != null
...
//				return schema.getDefaultMaxLimit() > -1; // 找不到则取schema的配置
      ...
      if ((tableConfig.isGlobalTable() && tableConfig.getDataNodes().size() == 1) ||
          (tableConfig.getRule() != null
              && tableConfig.getRule().getRuleAlgorithm().getDbCount() == 1 &&
              tableConfig.getRule().getRuleAlgorithm().getTbCount() == 1)) {
        return false;
      }
      ...
      return true;
    } else if (rrs.hasPrimaryKeyToCache()
        && ctx.getTables().size() == 1) {// 只有一个表且条件中有主键,不需要limit了,因为主键只能查到一条记录
      return false;
    } else {// 多表或无表
      return false;
    }
    ...
    for (int i = 0; i < groupByItems.size(); i++) {
      ...
      if (sqlExpr instanceof SQLIdentifierExpr) {
        ...
        if (expr instanceof SQLName) {
          column = StringUtil
              .removeBackquote(((SQLName) expr).getSimpleName());// 不要转大写 2015-2-10 sohudo
      ...
      /**
       * @// TODO: 2016/4/28 优化druid解析出函数列 对于含有函数的列，不能简单地取.之后的； 例SQL：select
       * h.id,DATE_FORMAT(h.times,'%b %d %Y %h:%i %p') from hotnews h GROUP BY
       * DATE_FORMAT(h.times,'%b %d %Y %h:%i %p'); 注意：这是一个临时方案，最好还是优化下druid
       * parser解析出函数列
       */
  ...
  protected LinkedHashMap<String, Integer> buildOrderByCols(List<SQLSelectOrderByItem> orderByItems,
    ...
    for (int i = 0; i < orderByItems.size(); i++) {
      ...
      map.put(col,
          type == SQLOrderingSpecification.ASC ? OrderCol.COL_ORDER_TYPE_ASC
              : OrderCol.COL_ORDER_TYPE_DESC);
    }
    return map;
  }

  protected void setLimitIFChange(SQLStatement stmt, RouteResultset rrs, SchemaConfig schema,
      SQLBinaryOpExpr one,
