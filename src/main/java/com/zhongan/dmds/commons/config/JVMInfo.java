...
import com.zhongan.dmds.commons.util.ReflectionProvider;
...
public class JVMInfo {
  ...
  /*
   * Support for sun.misc.Unsafe and sun.reflect.ReflectionFactory is present in
   * JRockit versions R25.1.0 and later, both 1.4.2 and 5.0 (and in future 6.0
   * builds).
   */
  private static boolean isBEAWithUnsafeSupport() {
    ...
    if (System.getProperty("java.vm.vendor").indexOf("BEA") != -1) {
      ...
      /*
       * Recent 1.4.2 and 5.0 versions of JRockit have a java.vm.version string
       * starting with the "R" JVM version number, i.e.
  ...
  protected boolean canUseSun14ReflectionProvider() {
    return
        (isSun() || isApple() || isHPUX() || isIBM() || isBlackdown() || isBEAWithUnsafeSupport())
            && is14()
