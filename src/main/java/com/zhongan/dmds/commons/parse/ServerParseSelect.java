...
public final class ServerParseSelect {
    ...
    switch (stmt.charAt(offset)) {
      ...
      default:
        if (CharTypes.isIdentifierChar(stmt.charAt(offset))) {
          for (; offset < stmt.length() && CharTypes.isIdentifierChar(stmt.charAt(offset));
              ++offset) {
            ;
          }
          return offset;
  ...
  public static int skipAs(String stmt, int offset) {
    offset = ParseUtil.move(stmt, offset, 0);
    if (stmt.length() > offset + "AS".length() && (stmt.charAt(offset) == 'A'
        || stmt.charAt(offset) == 'a')
   ...
   * @return index after identity or `identity` or "identity" or 'identity', might equals to length.
   * -1 if not identity or `identity` or "identity" or 'identity'
   */
  ...
  /**
   * select @@identity<br/> select @@identiTy aS iD
   */
