...
public class HintSQLHandler implements HintHandler {
  ...
  public RouteResultset route(SystemConfig sysConfig, SchemaConfig schema, int sqlType,
    ...
    RouteResultset rrs = routeStrategy
        .route(sysConfig, schema, hintSqlType, hintSQLValue, charset, sc, cachePool);
    ...
    for (int i = 0; i < newRrsNodes.length; i++) {
      newRrsNodes[i] = new RouteResultsetNode(
          oldRsNodes[i].getName() + DmdsConstants.seq_db_tb + oldRsNodes[i].getTableName(),
