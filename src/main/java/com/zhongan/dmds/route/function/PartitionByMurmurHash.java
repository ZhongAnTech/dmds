...
import com.zhongan.dmds.exception.MurmurHashException;
...
public class PartitionByMurmurHash extends AbstractPartitionAlgorithm implements RuleAlgorithm {
  ...
  public void init() {
    try {
      ...
      // boolean serializableBucketMap=bucketMapPath!=null &&
      // bucketMapPath.length()>0;
  ...
  // bucketMap.put(Integer.parseInt(entry.getKey().toString()),
   ...
   * 所有权重值必须是正整数，否则以1代替 @param weightMapPath @throws IOException @throws
   */
