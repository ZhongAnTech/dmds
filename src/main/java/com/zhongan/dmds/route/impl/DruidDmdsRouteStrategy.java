...
import com.zhongan.dmds.sqlParser.druid.*;
...
/**
 * Base on DruidMycatRouteStrategy
 */
public class DruidDmdsRouteStrategy extends AbstractRouteStrategy {

  public static final Logger LOGGER = LoggerFactory.getLogger(DruidDmdsRouteStrategy.class);
  ...
  public RouteResultset routeNormalSqlWithAST(SchemaConfig schema, String stmt, RouteResultset rrs,
    ...
    if (schema.isNeedSupportMultiDBType()) {
      parser = new DmdsStatementParser(stmt);
    } else {
      parser = new MySqlStatementParser(stmt);
    }

    DmdsSchemaStatVisitor visitor = null;
    ...
    try {
      ...
      visitor = new DmdsSchemaStatVisitor();
    } catch (Exception t) {
      LOGGER.error("DruidDmdsRouteStrategyError", t);
    ...
    if ((ctx.getTables() == null || ctx.getTables().size() == 0)
      ...
      return DmdsRouterUtil
          .routeToSingleNode(rrs, schema.getRandomDataNode(), druidParser.getCtx().getSql());
    }
    ...
    for (RouteCalculateUnit unit : druidParser.getCtx().getRouteCalculateUnits()) {
      RouteResultset rrsTmp = DmdsRouterUtil
          .tryRouteForTables(schema, druidParser.getCtx(), unit, rrs,
  ...
  public RouteResultset analyseShowSQL(SchemaConfig schema, RouteResultset rrs, String stmt)
    ...
    if (tabInd > 0) {// show tables
      int[] nextPost = DmdsRouterUtil.getSpecPos(upStmt, 0);
      if (nextPost[0] > 0) {// remove db info
        int end = DmdsRouterUtil.getSpecEndPos(upStmt, tabInd);
      ...
      if (!Strings.isNullOrEmpty(defaultNode)) {
        return DmdsRouterUtil.routeToSingleNode(rrs, defaultNode, stmt);
      }
      return DmdsRouterUtil.routeToMultiNode(false, rrs, schema.getMetaDataNodes(), stmt, null);
    }

    /**
     * show index or column
     */
    int[] indx = DmdsRouterUtil.getSpecPos(upStmt, 0);
    if (indx[0] > 0) {
      /**
       * has table
       */
      int[] repPos = {indx[0] + indx[1], 0};
      String tableName = DmdsRouterUtil.getShowTableName(stmt, repPos);
      /**
       * IN DB pattern
       */
      int[] indx2 = DmdsRouterUtil.getSpecPos(upStmt, indx[0] + indx[1] + 1);
      if (indx2[0] > 0) {// find LIKE OR WHERE
        repPos[1] = DmdsRouterUtil.getSpecEndPos(upStmt, indx2[0] + indx2[1]);

      }
      stmt = stmt.substring(0, indx[0]) + " FROM " + tableName + stmt.substring(repPos[1]);
      DmdsRouterUtil.routeForTableMeta(rrs, schema, tableName, stmt);
      return rrs;
     ...
     */
    int[] createTabInd = DmdsRouterUtil.getCreateTablePos(upStmt, 0);
    if (createTabInd[0] > 0) {
    ...
    return DmdsRouterUtil.routeToSingleNode(rrs, schema.getRandomDataNode(), stmt);
  }
  ...
  // findRouteWithcConditionsForOneTable(SchemaConfig schema, RouteResultset rrs,
  // Map<String, Set<ColumnRoutePair>> conditions, String tableName, String sql)
  // throws SQLNonTransientException {
  ...
  // LOGGER.debug("found partion nodes (using parent partion rule directly) for
  // child table to update "
  ...
  private static RouteResultset analyseDescrSQL(SchemaConfig schema, RouteResultset rrs,
    ...
    ind = pos;
    int[] repPos = {ind, 0};
    String tableName = DmdsRouterUtil.getTableName(stmt, repPos);
