...
import com.zhongan.dmds.commons.protocol.MysqlDefs;
...
import com.zhongan.dmds.net.util.ResultSetUtil;
import com.zhongan.dmds.route.Procedure;
import com.zhongan.dmds.route.ProcedureParameter;
...
/**
 * 管理端连接dmds-server用于操作使用
 * 2017.05 remove no usages found code
 */
public class JDBCConnection implements BackendConnection {
  ...
  private void executeSQL(RouteResultsetNode rrn, IServerConnection sc, boolean autocommit)
      throws IOException {
  ...
  private void executeddl(IServerConnection sc, String sql) throws SQLException {
    Statement stmt = null;
    try {
  ...
  private void ouputCallStatement(RouteResultsetNode rrn, IServerConnection sc, String sql)
      throws SQLException {
    ...
    try {
      ...
      if (procedure.getSelectColumns().size() > 0) {
        ...
        for (String name : procedure.getSelectColumns()) {
          ...
          curRow
              .add(StringUtil.encode(String.valueOf(stmt.getObject(procedureParameter.getIndex())),
                  sc.getCharset()));
        }
  ...
  private void ouputResultSet(IServerConnection sc, String sql) throws SQLException {
    ResultSet rs = null;
    Statement stmt = null;
    try {
      stmt = con.createStatement();
  ...
  public void execute(final RouteResultsetNode node, final IServerConnection source,
    ...
    DmdsContext.getInstance().getBusinessExecutor().execute(runnable);
  }
  ...
  public String toString() {
    return "JDBCConnection [id=" + id + ",autocommit=" + this.isAutocommit() + ",pool=" + pool
        + ", schema="
        + schema + ", dbType=" + dbType + ", oldSchema=" + oldSchema + ", packetId=" + packetId
        + ", txIsolation=" + txIsolation + ", running=" + running + ", borrowed=" + borrowed
        + ", host=" + host
        + ", port=" + port + ", con=" + con + ", respHandler=" + respHandler + ", attachement="
        + attachement
        + ", headerOutputed=" + headerOutputed + ", modifiedSQLExecuted=" + modifiedSQLExecuted
        + ", startTime="
