...
public class MySQLHeartbeat extends DBHeartbeat {
  ...
  private void switchSourceIfNeed(String reason) {
    ...
    if (switchType == DmdsConstants.NOT_SWITCH_DS) {
      if (LOGGER.isDebugEnabled()) {
        LOGGER.debug("not switch datasource ,for switchType is " + DmdsConstants.NOT_SWITCH_DS);
        return;
      }
      return;
    }
    IPhysicalDBPool pool = this.source.getDbPool();
    int curDatasourceHB = pool.getSource().getHeartbeat().getStatus();
    // read node can't switch ,only write node can switch
    if (pool.getWriteType() == DmdsConstants.WRITE_ONLYONE_NODE && !source.isReadNode()
      ...
      synchronized (pool) {
        ...
        if (curDatasourceHB != DBHeartbeat.INIT_STATUS
          ...
          IPhysicalDatasource[] allWriteNodes = pool.getSources();
          while (true) {
            if (nextId == curIndex) {
              break;
            }
            IPhysicalDatasource theSource = allWriteNodes[nextId];
            IDBHeartbeat theSourceHB = theSource.getHeartbeat();
            int theSourceHBStatus = theSourceHB.getStatus();
            if (theSourceHBStatus == DBHeartbeat.OK_STATUS) {
              if (switchType == DmdsConstants.SYN_STATUS_SWITCH_DS) {
