...
import com.zhongan.dmds.commons.stat.TableStat;
...
public class ShowSQLSumTable {
  ...
  public static void execute(ManagerConnection c, boolean isClear) {
    ...
    /*
     * int i=0; Map<String, TableStat> statMap =
     * TableStatAnalyzer.getInstance().getTableStatMap(); for (TableStat tableStat :
     * statMap.values()) { i++; RowDataPacket row = getRow(tableStat,i,
     * c.getCharset());//getRow(sqlStat,sql, c.getCharset()); row.packetId =
     * ++packetId; buffer = row.write(buffer, c,true); }
     */
    List<Map.Entry<String, TableStat>> list = TableStatAnalyzer.getInstance()
        .getTableStats(isClear);
    if (list != null) {
