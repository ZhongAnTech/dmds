...
public class ShowSysParam {
  ...
  private static final String[] PARAMNAMES = {
      ...
      "Dmds_processorCheckPeriod",
      "Dmds_dataNodeIdleCheckPeriod",
      "Dmds_dataNodeHeartbeatPeriod",
      "Dmds_slowTime",
      "Dmds_bindIp",
      "Dmds_serverPort",
      "Dmds_managerPort"
  ...
  private static final String[] PARAM_DESCRIPTION = {
      ...
      "主要用于指定NIOProcessor上共享的businessExecutor固定线程池大小。dmds在需要处理一些异步逻辑的时候会把任务提交到这个线程池中。新版本中这个连接池的使用频率不是很大了，可以设置一个较小的值。",
      "指定使用dmds全局序列的类型。", "指定Mysql协议中的报文头长度。默认4",
      ...
      "SQL执行超时的时间，dmds会检查连接上最后一次执行SQL的时间，若超过这个时间则会直接关闭这连接。默认时间为300秒",
      "清理NIOProcessor上前后端空闲、超时和关闭连接的间隔时间。默认是1秒",
      "对后端连接进行空闲、超时检查的时间间隔，默认是300秒", "对后端所有读、写库发起心跳的间隔时间，默认是10秒",
      "慢SQL的时间阀值，默认是1000毫秒",
      "dmds服务监听的IP地址，默认值为0.0.0.0", "dmds的使用端口，默认值为8066", "dmds的管理端口，默认值为9066"
  };

  public static final String[] ISOLATIONS = {
      "",
      "READ_UNCOMMITTED",
      "READ_COMMITTED",
      "REPEATED_READ",
